...
% 
%             tmp3 = delta(j,i)*F(k,fidx(n,j,i)); fprintf("F(%d,%d,%d) = % 3.1f, ", [k,j,i,tmp3]);
%             tmp3 = H(k,n*(i-1)+j); fprintf("H(%d,%d,%d) = % 3.1f\n", [k,j,i,tmp3]);
% %             fprintf("F(%d,%d,%d) = % 3.1f, F(%d,%d,%d) = % 3.1f, F(%d,%d,%d) = % 3.1f ", [i,j,k,tmp1,tmp2])
%             
%             CR_h = CR_h + H(i,n*(k-1)+j) + H(j,n*(k-1)+i) + H(k,n*(i-1)+j);
%             CR_f = CR_f + delta(j,k)*F(i,fidx(n,j,k)) + delta(i,k)*F(j,fidx(n,i,k)) + delta(j,i)*F(k,fidx(n,j,i));
% 
