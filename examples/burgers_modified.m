...
u_ref = ones(K,1);
[s_ref,A,B,F,Ai] = burgersFOM(N,dt,T_end,mu,u_ref);
...
err_int = zeros(length(r_vals),1);  % MODIFIED
...
function [s_all,A,B,F,ImdtA] = burgersFOM(N,dt,T_end,mu,u)
    dx = 1/(N-1);
    
    K = T_end/dt;
    
    [A,B,F] = getBurgersMatrices(N,dx,dt,mu);
    ImdtA = eye(N)-dt*A;
